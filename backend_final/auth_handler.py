...
def handler(event: Dict, context: Any) -> Dict:
    ...
    if "/login" in path: resp = _handle_login(body)
    elif "/validate" in path: resp = _handle_validate(body)
    elif "/revoke" in path: resp = _handle_revoke(body)
    else: resp = {"statusCode": 404, "body": json.dumps({"error": f"Not found: {path}"})}
