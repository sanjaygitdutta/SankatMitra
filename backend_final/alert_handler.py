...
from mitrashared.models import Alert, AlertDirection, UrgencyLevel, GPSCoordinate
from mitrashared.geo_utils import is_within_corridor, bearing_degrees
from mitrashared.bedrock_service import get_bedrock_service
...
def _handle_send(body: Dict) -> Dict:
    ...
    if vehicles:
        ...
        notification_data = {
            ...
            "ambulanceVehicleId": body.get("vehicleId", ""),  # So civilian app can poll live GPS
            "direction": direction.value,
            "etaSeconds": str(eta_seconds),
            "body": alerts.get("en", "Please clear the path"),
            "alerts": json.dumps(alerts),
            "ambulanceLat": str(ambulance_loc.latitude),
            "ambulanceLon": str(ambulance_loc.longitude),
        }
...
def handler(event: Dict, context: Any) -> Dict:
    ...
    logger.info(f"DEBUG: listdir /var/task: {os.listdir('/var/task')}")
    try:
        path = event.get("path", "")
        ...
        import traceback
        error_msg = f"Alert Lambda Error: {str(e)}\n{traceback.format_exc()}"
