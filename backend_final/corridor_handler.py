...
from mitrashared.models import (
    Corridor,
    CorridorRequest,
    CorridorStatus,
    CorridorUpdate,
...
from mitrashared.security import token_from_header, verify_token
...
corridor_table = dynamodb.Table(settings.dynamo_corridor_table)
...
def _handle_activate(body: Dict, vehicle_id: str) -> Dict:
    ...
    """
    try:
        req = CorridorRequest.model_validate({**body, "vehicle_id": vehicle_id})
    except Exception as e:
    ...
    if not route_waypoints:
        from mitrashared.geo_utils import haversine_distance
...
def _handle_list() -> Dict:
    try:
        ...
        # Enrich each corridor with the latest ambulance GPS position.
        # LocationHistory has composite key (vehicleId HASH + timestamp RANGE),
        # so we must use query() not get_item().
        import os as _os
        from boto3.dynamodb.conditions import Key as _Key
        location_table = dynamodb.Table(
            _os.environ.get("DYNAMO_LOCATION_TABLE", "SankatMitra-LocationHistory-v2")
        )
        enriched = []
        for item in items:
            corridor = dict(item)
            vehicle_id = corridor.get("emergencyVehicleId", "")
            if vehicle_id:
                try:
                    loc_resp = location_table.query(
                    ...
                    if loc_items:
                        loc = loc_items[0]
                        corridor["ambulanceLat"] = str(loc.get("latitude", ""))
                        corridor["ambulanceLon"] = str(loc.get("longitude", ""))
                        corridor["ambulanceHeading"] = str(loc.get("heading", "0"))
                except Exception as e:
                    logger.warning(f"Could not fetch GPS for {vehicle_id}: {e}")
            enriched.append(corridor)

        return {"statusCode": 200, "body": json.dumps({"corridors": enriched, "count": len(enriched)}, default=str)}
    except ClientError as e:
