...
from mitrashared.models import GPSCoordinate, GPSData, CellTowerInfo, SpoofingEvidence
...
def _handle_validate(body: Dict) -> Dict:
    ...
    try:
        ...
        cell_data = [CellTowerInfo(**t) for t in body.get("cellTowerData", [])]
        gps_data = GPSData(
            coordinate=coord,
            satellite_count=body.get("satelliteCount", 6),
            signal_strength=body.get("signalStrength", -80.0),
            cell_tower_data=cell_data or None,
        )
    except Exception as e:
